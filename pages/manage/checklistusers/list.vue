      ...
      <template v-slot:item="props">
        <div class="q-pa-xs col-xs-12">
          ...
          <q-card-actions align="right">
            <q-btn rounded color="primary" icon="edit" label="編集" :to="'/manage/checklistusers/' + props.row.id"/>
            <q-btn rounded color="red" icon="delete_forever" label="削除" @click="deleteChecklist(props.row.id, props.row.name)"/>
          </q-card-actions>
        </q-card>
        </div>
      </template>
